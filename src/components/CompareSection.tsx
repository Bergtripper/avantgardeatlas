...
export const CompareSection: React.FC<CompareSectionProps> = ({
  ...
  return (
    <section id="compare-section" className="w-full py-16 px-4 sm:px-6 lg:px-12 border-b border-[#E5E4DF] bg-[#FBFBFA]">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 p-4 bg-[#F5F4EE] border border-[#E5E4DF]">
                ...
                <button
                  ...
                  className={`px-3 py-1.5 font-mono text-xs cursor-pointer transition-all border ${
                    isSelected
                      ...
                      : 'bg-white text-[#525252] border-[#DDDCD4] hover:border-[#121212]'
                  }`}
                >
        ...
        <div className="overflow-x-auto pb-6">
          <div className="min-w-[900px]">
            {/* Columns Header Grid */}
            <div className={`grid grid-cols-12 gap-px bg-[#DDDCD4] border border-[#DDDCD4]`}>
              {/* Category Label Column */}
              <div className="col-span-3 bg-[#EBEAE4] p-4 text-xs font-mono uppercase tracking-wider text-[#737373] flex items-center">
                CATEGORY
              </div>
              ...
              {selectedMovements.map((m) => (
                <div
                  key={m.id}
                  className={`col-span-${Math.floor(9 / selectedMovements.length)} bg-[#FAF9F5] p-5 flex flex-col justify-between`}
                >
                  <div>
                    <span className="font-mono text-[10px] text-[#8C8C88] uppercase tracking-wider">
                      {m.period}
                    </span>
                    <h3 className="text-2xl font-bold tracking-tight text-[#121212] mt-1">
                      {m.name}
                    </h3>
                    <div className="text-xs text-[#525252] mt-1 font-mono">
                      {m.countries.join(', ')}
                    </div>
                  </div>
                  <button
                    onClick={() => onSelectMovement(m.id)}
                    className="mt-3 text-left font-mono text-[11px] underline text-[#121212] hover:text-[#D82B2B] cursor-pointer"
                  >
                    View Monograph →
                  </button>
                </div>
            ...
            <div className="grid grid-cols-12 gap-px bg-[#DDDCD4] border-x border-b border-[#DDDCD4]">
              <div className="col-span-3 bg-[#F5F4EE] p-4 text-xs font-mono font-semibold uppercase text-[#121212]">
                01 / CORE PHILOSOPHY
              </div>
              {selectedMovements.map((m) => (
                <div
                  key={m.id}
                  className={`col-span-${Math.floor(9 / selectedMovements.length)} bg-white p-4 text-xs text-[#333] leading-relaxed`}
                >
                  {m.coreIdeas}
                </div>
              ))}
            ...
            <div className="grid grid-cols-12 gap-px bg-[#DDDCD4] border-x border-b border-[#DDDCD4]">
              ...
              {selectedMovements.map((m) => (
                <div
                  key={m.id}
                  className={`col-span-${Math.floor(9 / selectedMovements.length)} bg-white p-4 text-xs text-[#333]`}
                >
                  <div className="font-mono text-xs font-semibold text-[#121212] mb-1">
            ...
            <div className="grid grid-cols-12 gap-px bg-[#DDDCD4] border-x border-b border-[#DDDCD4]">
              ...
              {selectedMovements.map((m) => (
                <div
                  ...
                  className={`col-span-${Math.floor(9 / selectedMovements.length)} bg-white p-4 text-xs text-[#333]`}
                >
                  <div className="font-semibold text-xs text-[#121212] mb-1">
                    {m.visualDna.typography.classification}
                  </div>
                  <div className="p-1.5 bg-[#F5F4EE] font-mono text-[11px] text-[#121212] truncate border border-[#DDDCD4] mb-2">
                    {m.visualDna.typography.specimen}
                  </div>
                  <ul className="text-[11px] text-[#666] space-y-0.5 list-disc list-inside">
            ...
            <div className="grid grid-cols-12 gap-px bg-[#DDDCD4] border-x border-b border-[#DDDCD4]">
              ...
              {selectedMovements.map((m) => (
                <div
                  key={m.id}
                  className={`col-span-${Math.floor(9 / selectedMovements.length)} bg-white p-4 text-xs text-[#444] leading-relaxed`}
                >
                  {m.architectureNotes}
                </div>
              ))}
            ...
            <div className="grid grid-cols-12 gap-px bg-[#DDDCD4] border-x border-b border-[#DDDCD4]">
              ...
              {selectedMovements.map((m) => {
                ...
                return (
                  <div
                    key={m.id}
                    className={`col-span-${Math.floor(9 / selectedMovements.length)} bg-white p-4`}
                  >
                    {work ? (
                      <div>
                        <ArchivalVectorPlate
                          ...
                          showLabels={false}
                        />
                        <div className="mt-2 font-semibold text-xs text-[#121212]">
                      ...
                      <div className="text-xs text-[#8C8C88] italic">No graphic specimen on file.</div>
                    )}
                  </div>
                );
              })}
            ...
            <div className="grid grid-cols-12 gap-px bg-[#DDDCD4] border-x border-b border-[#DDDCD4]">
              ...
              {selectedMovements.map((m) => {
                ...
                return (
                  <div
                    key={m.id}
                    className={`col-span-${Math.floor(9 / selectedMovements.length)} bg-white p-4 text-xs`}
                  >
                    <div className="flex flex-wrap gap-1">
                      {people.slice(0, 4).map((p, idx) => (
                        <span
                          key={p.id || idx}
                          className="font-mono text-[11px] px-1.5 py-0.5 bg-[#F5F4EE] border border-[#DDDCD4] text-[#121212]"
                        >
                          {p.name}
                        </span>
                      ))}
