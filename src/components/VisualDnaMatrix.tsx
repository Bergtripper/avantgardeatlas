...
export const VisualDnaMatrix: React.FC<VisualDnaMatrixProps> = ({
  ...
  const renderGeometryPreview = () => {
    switch (dna.geometry.diagramType) {
      case 'circle-square-triangle':
        return (
          <svg viewBox="0 0 160 60" className="w-full h-14 bg-[#F5F4EE] border border-[#DDDCD4]">
      ...
      case 'floating-planes':
        return (
          <svg viewBox="0 0 160 60" className="w-full h-14 bg-[#FAF9F5] border border-[#DDDCD4]">
  ...
  return (
    <div className="border border-[#E5E4DF] bg-[#FAF9F5] p-5">
      {/* Header */}
      <div className="border-b border-[#E5E4DF] pb-3 flex items-center justify-between">
        <div>
          <span className="font-mono text-[10px] uppercase tracking-widest text-[#8C8C88] block">
            VISUAL DNA PROFILE
          </span>
        ...
        <span className="font-mono text-[10px] text-[#A8A79E]">STANDARDIZED MATRIX</span>
      </div>

      <div className={`mt-4 grid ${isCompact ? 'grid-cols-1 gap-4' : 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6'}`}>
        {/* 1. Geometry */}
        <div className="border border-[#EAE9E4] bg-white p-3.5">
          <div className="font-mono text-[11px] font-semibold uppercase tracking-wider text-[#121212] mb-1.5 flex justify-between">
            <span>01 / GEOMETRY</span>
          </div>
          {renderGeometryPreview()}
          <div className="mt-2.5 flex flex-wrap gap-1">
            {dna.geometry.primaryShapes.map((shape, idx) => (
              <span key={idx} className="font-mono text-[11px] text-[#333] px-1.5 py-0.5 bg-[#F5F4EE] border border-[#E5E4DF]">
                {shape}
              </span>
            ))}
          </div>
          <p className="mt-2 text-xs text-[#666] leading-relaxed line-clamp-3">
        ...
        {/* 2. Composition */}
        <div className="border border-[#EAE9E4] bg-white p-3.5">
          <div className="font-mono text-[11px] font-semibold uppercase tracking-wider text-[#121212] mb-1.5">
            02 / COMPOSITION
          </div>
          <div className="font-semibold text-xs text-[#121212] mb-2 font-mono">
          ...
          <div className="grid grid-cols-4 sm:grid-cols-5 gap-1.5 my-2">
            {dna.colour.palette.map((c, idx) => (
              <div key={idx} className="group relative">
                <div
                  className="w-full h-8 border border-[#121212]/20 cursor-pointer"
                  style={{ backgroundColor: c.hex }}
                  title={`${c.name} (${c.hex})`}
                />
                <div className="mt-1 font-mono text-[9px] text-[#737373] truncate">
                  {c.hex}
                </div>
              </div>
            ))}
          </div>
          <p className="text-xs text-[#666] leading-relaxed mt-2 line-clamp-2">
        ...
        <div className="border border-[#EAE9E4] bg-white p-3.5">
          ...
          <div className="font-semibold text-xs text-[#121212] mb-2">
            {dna.typography.classification}
          </div>
          <div className="p-2 bg-[#F5F4EE] border border-[#DDDCD4] font-mono text-xs text-[#121212] tracking-wider truncate mb-2">
        ...
        <div className="border border-[#EAE9E4] bg-white p-3.5">
          ...
          <div className="flex flex-wrap gap-1.5 mt-2">
            {dna.materials.map((mat, idx) => (
              <span
                key={idx}
                className="font-mono text-xs px-2 py-1 bg-[#F5F4EE] border border-[#E5E4DF] text-[#333]"
              >
                {mat}
              </span>
            ))}
