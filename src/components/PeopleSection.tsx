...
export const PeopleSection: React.FC<PeopleSectionProps> = ({ onSelectMovement }) => {
  ...
  return (
    <section id="people-section" className="w-full py-16 px-4 sm:px-6 lg:px-12 border-b border-[#E5E4DF] bg-[#FBFBFA]">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Discipline Filters */}
        <div className="flex items-center gap-1 overflow-x-auto pb-3 mb-8 border-b border-[#E5E4DF]">
          <span className="font-mono text-xs text-[#8C8C88] uppercase mr-3 shrink-0">Discipline Filter:</span>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredFigures.map((fig) => {
            ...
            return (
              <div
                key={fig.id}
                className="border border-[#E5E4DF] bg-[#FAF9F5] p-6 flex flex-col justify-between hover:border-[#121212] transition-colors"
              >
                <div>
                  <div className="flex items-baseline justify-between text-xs font-mono text-[#737373] mb-1">
                    <span className="font-semibold text-[#121212]">{fig.years}</span>
                  ...
                  <div className="flex flex-wrap gap-1 my-3">
                    {fig.primaryMovements.map((mId) => (
                      <button
                        ...
                        className="font-mono text-[10px] uppercase px-2 py-0.5 bg-[#EBEAE4] hover:bg-[#121212] hover:text-white border border-[#D5D4CC] cursor-pointer transition-colors"
                      >
                        {mId}
                      </button>
                    ))}
                  ...
                  <p className="text-xs text-[#525252] leading-relaxed mt-3">
                    {fig.biography}
                  </p>

                  {/* Key Quote */}
                  {fig.keyQuote && (
                    <blockquote className="my-4 pl-3 border-l-2 border-[#121212] italic text-xs text-[#333]">
                      “{fig.keyQuote}”
                    </blockquote>
                  )}
                ...
                <div className="mt-6 pt-3 border-t border-[#E5E4DF] flex items-center justify-between text-xs font-mono">
                  <button
                    ...
                    className="underline text-[#737373] hover:text-[#121212] cursor-pointer"
                  >
                    {isExpanded ? 'Less ↑' : `More details (${fig.interventions.length}) ↓`}
                  </button>
                  <span className="text-[#A8A79E] text-[10px] uppercase">
